...
/** \addtogroup Algorithm
    ...
    @{*/
#ifndef NEVISDECODER_ALGO_FEM_DECODER_BASE_H
#define NEVISDECODER_ALGO_FEM_DECODER_BASE_H
...
namespace decoder {
  ...
  public:
	...
	module_address         = ::larlite::data::kINVALID_UCHAR;
	module_id              = ::larlite::data::kINVALID_UCHAR;
	event_number           = kINVALID_WORD;
	event_frame_number     = kINVALID_WORD;
	fem_trig_frame_number  = kINVALID_WORD;
	fem_trig_sample_number = kINVALID_WORD;
	nwords                 = 0;
	checksum               = 0;
	quality                = true;
      }
  ...
  public:
    ...
    /// Implementation of algo_base::process_word
    virtual inline FEMWord_t get_word_class(const UInt_t word) const {
      if     ( (word & 0xf000) == 0xf000 )
	return kFEM_HEADER;
      else if( (word & 0xf000) == 0x4000 )
	return kFEM_FIRST_WORD;
      else if( (word & 0xf000) == 0x9000 )
	return kCHANNEL_HEADER;
      else if( (word & 0xf000) == 0xa000 )
	return kCHANNEL_WORD;
      else if( (word & 0xf000) == 0xb000 )
	return kCHANNEL_LAST_WORD;
      else if( (word & 0xf000) == 0xc000 )
	return kFEM_LAST_WORD;
      else
	return kUNDEFINED_WORD;
    }
    ...
    virtual bool is_event_empty(){ return (_header_info.event_number == kINVALID_WORD);};
