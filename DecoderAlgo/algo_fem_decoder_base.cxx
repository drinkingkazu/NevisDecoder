#ifndef NEVISDECODER_ALGO_FEM_DECODER_BASE_CXX
#define NEVISDECODER_ALGO_FEM_DECODER_BASE_CXX
...
namespace decoder {
  ...
  //#################################################
  {
    ...
    _algo_fem_last_word = kFEM_LAST_WORD;
    reset();
  }
  ...
  //#################################################
  {
	...
	word_class == kFEM_HEADER &&
	last_word_class != kFEM_HEADER ) 
    ...
    case kUNDEFINED_WORD: 
      ...
      if(word != 0x0 || (last_word_class != kEVENT_LAST_WORD && last_word_class != kFEM_LAST_WORD) ) {
	...
	status = false;
      ...
      }else if(_verbosity[::larlite::msg::kINFO]){
	...
	if(_header_info.event_number>0)
	  ...
	  print(::larlite::msg::kWARNING,__FUNCTION__,Form("Failed decoding event %d ...",_header_info.event_number));

	print(::larlite::msg::kWARNING,__FUNCTION__,"DEBUG MODE => Continue to the next event...\n");
  ...
  //##################################################################################
  {
    ...
    if( word_class != kFEM_HEADER) {
    ...
    }else if(get_word_class(word>>16)!=kFEM_HEADER) {
      ...
      print(::larlite::msg::kERROR,__FUNCTION__,Form("Found an odd event header word: %x",word));
    ...
    if(status) {
      ...
      //
      FEMWord_t last_word_class = get_word_class(word);
      if( last_word_class == _algo_fem_last_word )

	store_event();

      else if( last_word_class != kFEM_HEADER && last_word_class != kEVENT_HEADER ) {
  ...
  //#################################################
    ...
    if(!( event_header[0] & 0xffff))
      print(::larlite::msg::kERROR,"Unexpected first word in event headers!");
	...
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Decoded Header: %s",msg.c_str()));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Module %d (ID=%d)", _header_info.module_address, _header_info.module_id));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Event ID %d",_header_info.event_number));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Frame ID %d",_header_info.event_frame_number));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Number of Words = %d",_header_info.nwords));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Checksum = %x", _header_info.checksum));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Trigger Frame %d",_header_info.fem_trig_frame_number));
	print(::larlite::msg::kINFO, __FUNCTION__, Form("Trigger Sample %d",_header_info.fem_trig_sample_number));
      }
  ...
  //#################################################
  {
    ...
    //    else if (nbits==4) {nbits=3; diff = 0x7; mask = 0x7;}
      ...
      print(::larlite::msg::kERROR,__FUNCTION__,"Only supported for nbits = [3,4]!");
	...
	print(::larlite::msg::kERROR,__FUNCTION__,Form("Unexpected diff: ref=%d, subject=%d",ref,subject));
	throw decode_algo_exception();
      }
